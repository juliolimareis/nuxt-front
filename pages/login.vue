    ...
    <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
      <div class="space-y-6">
        ...
        <p v-if="statusResponse > 0" class="mt-10 text-center text-sm text-gray-600 bg-red-100 rounded-sm p-4 transform">
          ...
          {{ ' ' }}
          <NuxtLink href="#" class="font-semibold leading-6 text-primary-500">{{ 'Recovery password' }}</NuxtLink>
        </p>
      ...
      <p class="mt-10 text-center text-sm text-gray-500">
        {{ "Not a member?" }}
        {{ ' ' }}
        <NuxtLink href="/register" class="font-semibold leading-6 text-primary-500 hover:text-primary-500">{{ 'Create Account' }}</NuxtLink>
      </p>
    </div>
  </div>
</template>
...
const rules = computed(() => ({
  ...
  passwd: {
    required: helpers.withMessage("The password must contain at least 6 characters", required),
